    ...
    bury:(type)->
        ...
        switch @rule.heavenview
            when "view"
                @heavenview=true
            when "norevive"
                @heavenview = @rule.jobrule != "特殊规则.Endless黑暗火锅" && !@players.some((x)->x.isReviver())
            else
                @heavenview=false
        ...
        deads.forEach (obj)=>
            ...
            situation=switch obj.found
                #死因
                when "werewolf","werewolf2","trickedWerewolf","poison","hinamizawa","vampire","vampire2","witch","dog","trap","bomb","marycurse","psycho","crafty","lunaticlover","hooligan"
...
getrulestr = (i18n, rule, jobs={})->
    ...
    if rule in ["特殊规则.黑暗火锅", "特殊规则.Endless黑暗火锅"]
